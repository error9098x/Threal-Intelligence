...
export default function MaliciousIpFeed() {
  ...
  const loadThreatFoxData = async () => {
    try {
      ...
      if (data && data.data && data.data.length > 0) {
        // Filter for IP IOCs
        const ipIocs = data.data.filter((ioc: IOC) => ioc.ioc_type === "ip" || ioc.ioc_type === "ip:port")
        setIocs(ipIocs)
        setVisibleIocs(ipIocs.slice(0, page * IOC_PAGE_SIZE))
      } else {
        setIocs([])
        setVisibleIocs([])
      }
      ...
      setError("Failed to fetch malicious IP data. Please try again later.")
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-blue-100">Malicious IP Feed</h2>
      ...
      {loading && iocs.length === 0 && (
        <div className="flex justify-center items-center py-12">
          ...
          <span className="ml-2 text-blue-300">Loading malicious IP data...</span>
        </div>
      )}
      ...
      <div className="space-y-4">
        {visibleIocs.length > 0 ? (
          ...
          <div className="text-center py-8 text-blue-300">No malicious IPs detected in the last 5 minutes.</div>
        ) : null}
      </div>
